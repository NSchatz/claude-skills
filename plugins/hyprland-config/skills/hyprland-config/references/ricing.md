...
windowrule = opaque on, match:class steam

# Floating windows slightly transparent
windowrule = opacity 0.9 override, match:float true

# Keep file manager opaque
windowrule = opacity 1.0 override 1.0 override, match:class thunar
```
...
```ini
# Frosted glass effect on waybar, wofi, notifications
layerrule = blur on, match:namespace waybar
layerrule = ignorezero on, match:namespace waybar    # don't blur transparent pixels
layerrule = blur on, match:namespace wofi
layerrule = blur on, match:namespace rofi
layerrule = ignorezero on, match:namespace rofi
layerrule = blur on, match:namespace notifications   # dunst/mako
...
4. **Missing `layerrule = blur on`** for bars/launchers — windows blur but waybar stays flat.
